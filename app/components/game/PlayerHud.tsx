...
// Heads-up display showing the player's character info and resources.
...
export default function PlayerHud({ player }: PlayerHudProps) {
    ...
    return (
        ...
        <span
            ...
            style={{
                background: player.faction === "Alliance" ? "#2563eb" : "#dc2626",
            }}
        />
          ...
          {player.health.current}/{player.health.maximum}
            ...
            {Object.entries(player.resources).map(([key, pool]) => {
                const pct =
                    pool.maximum > 0 ? (pool.current / pool.maximum) * 100 : 0;
                return (
              ...
              {key.slice(0, 2).toUpperCase()}
                        ...
                        <div className="player-hud__bar-track">
                            <div
                                ...
                                style={{
                                    width: `${pct}%`,
                                    background: RESOURCE_COLORS[key] ?? "#6b7280",
                                }}
                            />
                        </div>
                        <span className="player-hud__bar-value">
